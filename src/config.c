...
int loadGameConfig(char *file, int *levels, int *difficulty, int *fullscreen, int *volume, int *lives, int *intNorm, int *intEasy, int *intNormBoss, int *intEasyBoss, int *framerate, int *showfps, int *showHUD, int *drops,int *dropSeed, int *parts, int *powerMax, struct spriteGroup *drop, int *playerParts, struct spriteGroup *player)
{
	FILE *config;
	int len=0;
	char inbyte[128];
	...
	*(levels)=strtol(inbyte,NULL,10);
	...
	inbyte[len]='\0';
	isValid(config,file,len);
	*(framerate)=strtol(inbyte,NULL,10);
	len=0;
	...
	*(showHUD)=strtol(inbyte,NULL,10);
	...
	inbyte[len]='\0';
	isValid(config,file,len);
	*(difficulty)=strtol(inbyte,NULL,10);
	len=0;
	...
	inbyte[len]='\0';
	isValid(config,file,len);
	*(fullscreen)=strtol(inbyte,NULL,10);
	len=0;
	...
	*(lives+1)=strtol(inbyte,NULL,10);
	...
	inbyte[len]='\0';
	isValid(config,file,len);
	*(lives+2)=strtol(inbyte,NULL,10);
	len=0;
    
    read(config,inbyte,&len);	// Read interval add normal enemy
	inbyte[len]='\0';
	isValid(config,file,len);
	*(intNorm)=strtol(inbyte,NULL,10);
	len=0;
    
	read(config,inbyte,&len);	// Read inteval add easy enemy
	inbyte[len]='\0';
	isValid(config,file,len);
	*(intEasy)=strtol(inbyte,NULL,10);
	len=0;
    
    read(config,inbyte,&len);	// Read interval add normal boss
	inbyte[len]='\0';
	isValid(config,file,len);
	*(intNormBoss)=strtol(inbyte,NULL,10);
	len=0;
    
	read(config,inbyte,&len);	// Read inteval add easy boss
	inbyte[len]='\0';
	isValid(config,file,len);
	*(intEasyBoss)=strtol(inbyte,NULL,10);
	len=0;
	...
	*(powerMax)=strtol(inbyte,NULL,10);
	...
	for(l=0;l<*(powerMax);l++)
	{
		...
		for(d=0;d<player->part[l].firing;d++)
		{
			...
			inbyte[len]='\0';
			isValid(config,file,len);
			player->part[l].firespeed[d]=strtol(inbyte,NULL,10);
			len=0;	
...
int loadBossConfig(char *file, int *parts, int *bossX, int *bossY, struct bossPart *piece, int *desGroups, int *hpNorm, int *hpHard, struct bossHit *hits, int *startSpeed, int *loopSpeed)
{
	FILE *config;
	int len=0;
	char inbyte[128];
	...
	*(hpHard)=strtol(inbyte,NULL,10);
	...
	for(d=0;d<=*(desGroups);d++)
	{
		...
		read(config,inbyte,&len);	// read start movment speed
		inbyte[len]='\0';
		isValid(config,file,len);
		*(startSpeed)=strtol(inbyte,NULL,10);
		len=0;

		read(config,inbyte,&len);	// read loop movment speed
		inbyte[len]='\0';
		isValid(config,file,len);
		*(loopSpeed)=strtol(inbyte,NULL,10);
		len=0;
		...
		if ( piece[d].frames > 0 )
		{
			...
			if ( piece[d].firing > 0 )
			{
				for(e=0;e<piece[d].firing;e++)
				{
					...
					inbyte[len]='\0';
					isValid(config,file,len);
					piece[d].firespeed[e]=strtol(inbyte,NULL,10);
					len=0;
...
int loadLevelConfig(char *file, int *backgrounds, int *bgLoop, int *enemies, int *enemySeed, int *bulletSeed, int *hitsEasy, int *hitsNorm, int *hitsHard,int *parts,int *speed, struct spriteGroup *enemy)
{
	FILE *config;
	int len=0;
	char inbyte[128];
...
/*
			...
			if ( enemy[d].part[e].firing > 0 )
			{
				...
				for(l=0;l<enemy[d].part[e].firing;l++)
				{
					...
					inbyte[len]='\0';
					isValid(config,file,len);
					enemy[d].part[e].firespeed[l]=strtol(inbyte,NULL,10);
					len=0;
