...
int shuffleEnemyBulletCollide(int *count, int *type, int *dist, int *start, int *speed, int *wait, int *part, int *fire, int *objX, int *objY, struct coords *area, int *collide, int collisions)
{
	int d,e;
...
int shuffleEnemyBullet(int *count, int *type, int *dist, int *start, int *speed, int *wait, int *part, int *fire, int *objX, int *objY, int *objW, int *objH, struct coords *area)
{
	int d,e;
	int shuffle=0;
    int count2=*(count);
...
int shuffleEnemyHit(int *hits, int *collide, int *collisions, int power)
{
	...
	for(l=0;l<*(collisions);l++)
	{
		if ( *(hits+*(collide+l)) > 0 )
		{
			*(hits+*(collide+l))=*(hits+*(collide+l))-(power+1);
            if ( *(hits+*(collide+l)) >= 0 )
            {
                *(collisions)=*(collisions)-1;
                remCollide=1;
